...
import { localProjectData } from "@/data/index";
import ProjectCard from "@/components/admin/project-card";
import { cn } from "@/lib/utils";
import { useRouter } from "next/navigation";
import { Button } from "@/components/ui/button";
import { onGetProjectList } from "@/actions/projects";
import { ProjectPaginationType, ProjectType } from "@/schema/projects.schema";
...
const ProjectsPage = () => {
  ...
  const fetchData = async (curPage: number) => {
    ...
    const response = await onGetProjectList(curPage, perPage) as ProjectPaginationType
    if (!response) return;
    setData(response?.data as ProjectType[]);
    ...
    <div className="flex flex-col gap-4 relative">
      ...
      flex items-center justify-between border-b">
        ...
        <div className="flex flex-row gap-4">
          <Button variant={"outline"} onClick={() => router.push("/projects/create")} className="ml-auto">Add New</Button>
